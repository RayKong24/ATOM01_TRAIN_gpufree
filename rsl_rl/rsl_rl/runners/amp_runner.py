...
class AMPRunner(OnPolicyRunner):
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        if isinstance(obs, tuple):
            if isinstance(obs[1], dict) and "observations" in obs[1]:
                obs = TensorDict(obs[1]["observations"], batch_size=[self.env.num_envs])
            else:
                obs = obs[0]
        ...
        for it in range(start_it, total_it):
            start = time.time()  # <--- 添加这行记录开始时间
            with torch.inference_mode():
                # Rollout
                for _ in range(self.cfg["num_steps_per_env"]):  # <--- 将 self.num_steps_per_env 修改为 self.cfg["num_steps_per_env"]
